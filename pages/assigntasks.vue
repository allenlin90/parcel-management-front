<template>
    <section class="margin_x">
            ...
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-select
                        ...
                        item-text="name"
                        item-value="id"
                        v-model="selectedParcel.toDriverId"
                    >
                    </v-select>
                </v-card-text>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="error" @click="resetDialog">Cancel</v-btn>
                    <v-btn color="primary" @click="confirmChangeDriver">
                        Change Driver
                    </v-btn>
                </v-card-actions>
            </v-card>
        ...
        <v-app-bar fixed app style="top: 56px; overflow: auto" color="#616161">
            <v-container>
                <v-row>
                    <v-col class="text_white"
                        >Total Tasks:
                        {{ this.tasks.length + this.assignedTasks }}</v-col
                    >
                    <v-col class="text_white"
                        >Unassigned Tasks: {{ this.tasks.length }}</v-col
                    >
                    <v-col class="text_white"
                        >Assigned Tasks: {{ this.assignedTasks }}</v-col
                    >
                </v-row>
            </v-container>
        </v-app-bar>
        ...
        <v-row>
            <v-card>
                ...
                >
                    ...
                    <template v-slot:header.remove> </template>
                    <template v-slot:body>
                        ...
                        >
                            <tr
                                ...
                                :style="{
                                    display: filteredParcels.includes(
                                        item.trackingId
                                    )
                                        ? ''
                                        : 'none',
                            ...
                            >
                                <td>
                                    <input
                                        ...
                                        @change="selectParcel(item.trackingId)"
                                    />
                                </td>
                                <td>{{ index + 1 }}</td>
            ...
            >
                            ...
                            <v-col cols="8">
                                <v-row>
                                    ...
                                    <v-col cols="4">
                                        <v-btn
                                            color="error"
                                            @click="clearDriverTasks(driver.id)"
                                            >Clear Tasks</v-btn
                                        >
                                    </v-col>
                                    <v-col cols="4">
                ...
                <template v-slot:body>
                    <draggable
                        ...
                        @end="drag = false"
                    >
                        <tr v-for="(item, index) in driver.tasks" :key="index">
                            <td>
                                <v-icon
                                    small
                                    class="page__grab-icon"
                                    @click.prevent="
                                        changeDriver(item.trackingId, driver.id)
                                    "
                                >
                                    mdi-arrow-all
                                </v-icon>
                            ...
                            <td>{{ item.deliveryStatus }}</td>
                            <td>{{ item.parcelType }}</td>
                            <td class="align_cell_center">
                                <v-icon
                                    @click="
                                        removeTask(item.trackingId, driver.id)
                                    "
                                >
                                    mdi-close-thick
                                </v-icon>
...
export default {
    ...
    computed: {
        ...
        assignedTasks() {
            const countAssignedTasks = this.drivers.reduce(
                (sumTasks, driver) => {
    ...
    methods: {
        confirmChangeDriver() {
            const { fromDriverId, toDriverId, trackingId } = this.selectedParcel
            const fromDriverIndex = this.drivers.findIndex(
                (driver) => driver.id === fromDriverId
            )
            const toDriverIndex = this.drivers.findIndex(
                (driver) => driver.id === toDriverId
            )
            if (fromDriverId !== toDriverId) {
                const taskIndexFrom = this.drivers[
                    this.drivers.findIndex(
                        (driver) => driver.id === fromDriverId
                    )
                ].tasks.findIndex((task) => task.trackingId === trackingId)
                this.drivers[toDriverIndex].tasks.push(
                    this.drivers[fromDriverIndex].tasks[taskIndexFrom]
                )
                this.drivers[fromDriverIndex].tasks.splice(taskIndexFrom, 1)
            }
            this.dialog = false
        },
        changeDriver(trackingId, driverId) {
            this.dialog = true
            this.selectedParcel.trackingId = trackingId
            this.selectedParcel.fromDriverId = driverId
            this.selectedParcel.toDriverId = driverId
        },
        assignToDriver() {
            ...
            this.tasks = this.tasks.filter(
                (task) => !allTrackingId.includes(task.trackingId)
            )
        ...
        removeTask(trackingId, driverId) {
            ...
            const task = this.drivers[driverIndex].tasks.find(
                (task) => task.trackingId === trackingId
            )
            const taskIndex = this.drivers[driverIndex].tasks.findIndex(
                (task) => task.trackingId === trackingId
            )
